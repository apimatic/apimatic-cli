...
export class Color {
  ...
  private static parseFunction(text: string): Rgb | undefined {
    ...
    return match[1].toLowerCase().startsWith('rgb') ? Color.rgbChannels(parts.values) : Color.hslChannels(parts.values);
  }

  /** Both the legacy comma syntax and the space syntax with an optional `/ alpha`. */
  private static channels(inner: string): { values: string[]; alpha: string | undefined } | undefined {
    const trimmed = inner.trim();
    if (trimmed.includes(',')) {
      const values = trimmed.split(',').map((part) => part.trim());
      return values.length === 3 || values.length === 4 ? { values: values.slice(0, 3), alpha: values[3] } : undefined;
    }
    const [main, alpha, ...rest] = trimmed.split('/').map((part) => part.trim());
    const values = main.split(/\s+/);
    return rest.length === 0 && values.length === 3 ? { values, alpha } : undefined;
  }

  private static rgbChannels(values: string[]): Rgb | undefined {
  ...
  private static hslChannels([hue, saturation, lightness]: string[]): Rgb | undefined {
    const degrees = hue.toLowerCase().endsWith('deg') ? hue.slice(0, -3) : hue;
    const s = PERCENTAGE.exec(saturation);
    const l = PERCENTAGE.exec(lightness);
    if (!NUMBER.test(degrees) || s === null || l === null) {
      return undefined;
    }
    const sFraction = Color.within(Number(s[1]), 100);
    const lFraction = Color.within(Number(l[1]), 100);
    if (sFraction === undefined || lFraction === undefined) {
      return undefined;
    }
    return hslToRgb(Number(degrees), sFraction, lFraction);
  }
