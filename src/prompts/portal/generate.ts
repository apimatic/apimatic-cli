...
export class PortalGeneratePrompts {
    ...
    async existingDestinationPortalFolderPrompt(): Promise<void> {
    ...
    async existingDestinationPortalZipPrompt(): Promise<void> {
        ...
        if (useExistingZip === "no") {
            outro(
                "Please enter a different destination path or delete the existing zip file and try again."
            );
            process.exit(0);
        }
    }
