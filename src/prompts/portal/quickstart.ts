...
import { getMessageInCyanColor, getMessageInGreenColor, getMessageInOrangeColor, getMessageInMagentaColor, getMessageInRedColor, isValidUrl, directoryToJson } from '../../utils/utils';

export class PortalQuickstartPrompts {
  private spin = spinner();
  private vscodeExtensionUrl = '\u001b]8;;https://marketplace.visualstudio.com/items?itemName=apimatic-developers.apimatic-for-vscode\u001b\\APIMatic\'s interactive VS Code Extension\u001b]8;;\u001b\\';
  private serverUrl = '\u001b[4mhttp://localhost:3000\u001b[0m';
  private referenceDocumentation = '\u001b]8;;https://docs.apimatic.io/platform-api/#/http/guides/generating-on-prem-api-portal/overview-generating-api-portal\u001b\\\u001b[4mreference documentation\u001b[0m\u001b]8;;\u001b\\';
  private customizeTheSdks = '\u001b]8;;https://docs.apimatic.io/generate-sdks/codegen-settings/codegen-settings-overview/\u001b\\\u001b[4mcustomize the SDKs\u001b[0m\u001b]8;;\u001b\\';
  ...
  async loginPrompt(): Promise<{ email: string, password: string}> {
    ...
    const email = await text({
      ...
      validate: (input) => {
      if (!input) {
          throw new Error(getMessageInRedColor("Email is required."));
      }
      }
    });
    ...
    const pass = await password({
      ...
      validate: (input) => {
      if (!input) {
          throw new Error(getMessageInRedColor("Password is required."));
      }
      }
    });
    ...
    const spec = await text({
      ...
      validate: (input) => {
        if (!isValidUrl && !fs.existsSync(path.resolve(input)) ) {
          throw new Error(getMessageInRedColor("The directory path does not exist."));
        }
      }
    });
  ...
  displayOutroMessage(): void {
    log.step(
      ...
      getMessageInCyanColor(`- Read the ${this.referenceDocumentation}`) + getMessageInCyanColor(` to learn more about how you can customize this API Portal.\n`) +
      ...
      getMessageInCyanColor(`- Check out how you can ${this.customizeTheSdks}`) + getMessageInCyanColor(` using Code Generation settings.\n`)
    );
  }
}