...
Let's get started!`);
  ...
  /** The project came with its own source directory, so there is nothing to import into it. */
  public importSpecStepAdopted(sourceDirectory: DirectoryPath) {
    log.info(`Step 1 of 3: Using the OpenAPI Definition already in ${f.path(sourceDirectory)}`);
  }
  ...
  // Everything is checked: a first portal covering every language it can is the answer one Enter gives.
  ...
  public gitignoreNotUpdated(failure: GitignoreFailure, projectDirectory: DirectoryPath) {
    const problem = failure === 'unreadable' ? 'read' : 'written';
    const message =
      `${f.var(GITIGNORE)} in ${f.path(projectDirectory)} could not be ${problem}, so ` +
      `${GENERATED.map((entry) => f.var(entry)).join(', ')} were not added to it. ` +
      `Add them yourself, or Git will track everything the CLI generates.`;
    log.warn(message);
  }
}
