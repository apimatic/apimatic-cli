...
import { discardStreamBody, replaceHTML } from '../../src/utils/utils.js';

// The server writes validation messages for its web page; the terminal gets their text.
describe('replaceHTML', () => {
  it("keeps a link's address and puts each line break on a line of its own", () => {
    const html =
      'Missing description. (<a href="https://docs.apimatic.io/x" target="_blank">View Details</a>)' +
      '<br/><b>Source</b>: <i><code>API</code></i>.<br> <br/>';

    expect(replaceHTML(html)).to.equal('Missing description. (https://docs.apimatic.io/x)\nSource: API.');
  });

  it('leaves plain text as it is', () => {
    expect(replaceHTML('Schema should include a description.')).to.equal('Schema should include a description.');
  });
});
