...
import { ContentFile, ContentTree } from '../../../src/types/portal/content-tree';
...
describe('ContentTree', () => {
  ...
  it('judges the files it is handed', async () => {
    ...
    const checked = await tree.check({ pages, navigationFiles }, [], GENERATED);
  ...
  it('names the tabs from what it is handed', async () => {
    const navigationFiles = handed(
      [inContent('nav.json'), JSON.stringify({ title: 'Guides', pages: ['index', 'guides'] })],
      [inContent('guides', 'nav.json'), JSON.stringify({ title: 'Guides' })]
    );

    const notices = (await tree.check({ pages, navigationFiles }, [], GENERATED))._unsafeUnwrap();
